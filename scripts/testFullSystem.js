...
async function main() {
  ...
  try {
    ...
    if (!sbOk) {
      report(1, false, `Supabase: ${debug.body.supabase || debug.status}`);
    } else {
      ...
      report(
        1,
        existing >= 5,
        `Supabase OK. Tables checked: ${Object.entries(tables)
          .map(([k, v]) => `${k}=${v}`)
          .join(', ')}`
      );
    }
  } catch (e) {
    report(1, false, e.message);
  }
  ...
  try {
    ...
    if (!pinecone || !openai) {
      ...
      const mem = await api('POST', '/webhook/brand-memory-retrieve', {
        business_id: USER,
      ...
      report(
        2,
        mem.status === 200,
        `Pinecone connected. Brand memory query returned ${count} examples. ${mem.body?.reason || ''}`
      );
    }
  } catch (e) {
    report(2, false, e.message);
  }
  ...
  try {
    ...
    if (!profile.body?.found) {
      ...
      report(
        4,
        hasName && hasType,
        `Profile: ${p.business_name} (${p.business_type}). Master prompt will include business context, location, audience, products, rules.`
      );
    }
  } catch (e) {
    report(4, false, e.message);
  }
  ...
  try {
    ...
    if (gen.status !== 200) {
      report(5, false, `ideas/generate returned ${gen.status}`);
    } else {
      ...
      report(
        5,
        proper.length >= 3,
        `${list.length} ideas total, ${proper.length} properly parsed, ${hasCategory} with category, ${hasPriority} with priority, ${hasExecute} with execution steps`
      );
    }
  } catch (e) {
    report(5, false, e.message);
  }
  ...
  try {
    ...
    report(
      6,
      intel.status === 200,
      `${total} intelligence entries from ${modules.length} modules: ${modules.join(', ') || 'none yet'}`
    );
  } catch (e) {
    report(6, false, e.message);
  }
  ...
  try {
    ...
    if (run.status !== 200) {
      report(7, false, `Orchestrator returned ${run.status}`);
    } else {
      ...
      if (logList.length === 0) {
        ...
        const planned = (() => {
          try {
            return JSON.parse(latest.tasks_planned || '[]').length;
          } catch {
            return 0;
          }
        })();
        const executed = (() => {
          try {
            return JSON.parse(latest.tasks_executed || '[]').length;
          } catch {
            return 0;
          }
        })();
        report(
          7,
          true,
          `Orchestrator: ${planned} planned, ${executed} executed. Report: ${(latest.report || '').slice(0, 100)}`
        );
      }
    }
  } catch (e) {
    report(7, false, e.message);
  }
  ...
  try {
    ...
    report(
      8,
      typeof score === 'number' && health.status === 200,
      `Health score: ${score}/100. Opportunities: ${opCount}. Recommendations: ${recs.length}`
    );
  } catch (e) {
    report(8, false, e.message);
  }
  ...
  try {
    ...
    report(
      9,
      r.status === 200,
      `Memory system accessible. ${memEntries} memory entries. Metrics: ${r.body?.posts_this_week || 0} posts this week, trend=${r.body?.trend || '?'}`
    );
  } catch (e) {
    report(9, false, e.message);
  }
  ...
  try {
    ...
    report(
      10,
      country === 'XK' && validTime,
      `Country: ${country} (${countryData.name}). ${holidays.length} holidays in 60d. Next post: ${postTime?.slice(0, 16) || '?'}. Languages: ${countryData.languages.join(',')}`
    );
  } catch (e) {
    report(10, false, e.message);
  }
  ...
  console.log(
    `  System ready for production: ${failed <= 2 && critical.length === 0 ? 'YES ✅' : 'NEEDS ATTENTION ⚠️'}`
  );
  console.log('═══════════════════════════════════════════════');
}
