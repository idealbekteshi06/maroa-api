...
const SEEDS = [
  ...
  },
  {
    ...
    body:
      "Open at 6am. The line starts at 5:55am. We don't make it shorter — we make every cup worth standing for.",
    cta: 'Find your store',
    visual_brief:
  ...
  // ═══ Pret style — chains with taste-of-place ═══
  {
    ...
    body:
      "Five drinks a day, every day. £30 a month. Bring the friend who keeps borrowing from your account.",
    cta: "Get Club Pret",
    visual_brief:
...
async function main() {
  ...
  if (DRY_RUN) {
    for (const s of SEEDS) {
      console.log(
        `  ${s.source_ref}  ${s.region} ${s.format} — ${s.title.slice(0, 60)} (q=${s.quality_score})`,
      );
    }
