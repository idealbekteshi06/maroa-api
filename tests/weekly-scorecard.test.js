...
test('buildScorecardData: aggregates spend + clicks correctly', () => {
  ...
  const r = sc.buildScorecardData({
    ...
    prevWeekRows: [],
    campaigns: [
      { id: 'c1', business_name: 'A' },
      { id: 'c2', business_name: 'B' },
    ],
  });
...
test('buildScorecardData: identifies best/worst campaign by ROAS', () => {
  ...
  const r = sc.buildScorecardData({
    ...
    prevWeekRows: [],
    campaigns: [
      { id: 'a', business_name: 'Alpha' },
      { id: 'b', business_name: 'Beta' },
    ],
  });
...
test('buildScorecardData: computes WoW deltas', () => {
  ...
  const r = sc.buildScorecardData({ thisWeekRows, prevWeekRows, campaigns: [] });
  assert.ok(r.deltas.spend_pct > 0.2 && r.deltas.spend_pct < 0.3, 'spend delta should be ~25%');
  assert.ok(r.deltas.roas_pct > 0.3, 'ROAS delta should be ~33%');
});
