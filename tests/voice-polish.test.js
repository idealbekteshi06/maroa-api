...
test('detect: high specificity reduces should_rewrite even with some slop', () => {
  const r = vp.detect(
    'Open Mon-Fri 9-17 at Rruga Myslym Shyri 5, Tirana. €5 espresso, €8 cappuccino. Call +355 69 123 4567.'
  );
...
test('detect: clean concrete copy has low slop score', () => {
  const r = vp.detect(
    'Open 9-17 Mon to Fri. Espresso €1.50. Call us at +355 69 123 4567 or come visit at Rruga Myslym Shyri 5.'
  );
...
test('shouldRewrite: high slop + low specificity → rewrite', () => {
  const r = vp.detect(
    "Leverage our world-class cutting-edge innovative solutions to elevate your business and unlock the power of synergy in today's fast-paced world."
  );
  ...
  const r = await vp.polish({
    ...
    callClaude: async () =>
      JSON.stringify({
        ...
        polished:
          "In today's fast-paced world, you must leverage world-class cutting-edge innovative solutions to elevate your business.",
        changes_made: ['none'],
        language_preserved: true,
      }),
    extractJSON: JSON.parse,
