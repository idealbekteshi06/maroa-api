...
test('modelsFor: agency=Opus exec, growth=Sonnet 5 exec + Opus advisor, free=Sonnet only', () => {
  assert.strictEqual(advisor.modelsFor('agency').executor, 'claude-opus-4-8');
  assert.strictEqual(advisor.modelsFor('growth').executor, 'claude-sonnet-5');
  assert.strictEqual(advisor.modelsFor('growth').advisor, 'claude-opus-4-8');
...
test('callWithAdvisor: routes through callClaude with advisor extras when enabled', async () => {
  ...
  assert.strictEqual(captured.extra.advisor.model, 'claude-opus-4-8');
