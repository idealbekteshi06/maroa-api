...
function buildStrategicSynthesisPrompt(ctx, bundle) {
  const addendum = `
...
function buildClientVoicePrompt(ctx, synthesis) {
  const addendum = `
...
function renderBundle(b) {
  ...
  for (const p of b.platforms) {
    ...
    if (p.bottomPost)
      lines.push(`    bottom: "${p.bottomPost.hook}" @ ${(p.bottomPost.engagementRate * 100).toFixed(1)}%`);
  }
  lines.push('');
  lines.push('ADS');
  ...
  if (b.email) {
    ...
    lines.push('EMAIL');
    lines.push(
      `  sent=${fmt(b.email.sent)}; open=${fmt(b.email.openRate)}; click=${fmt(b.email.clickRate)}; rev=${fmt(b.email.revenueAttributed, ' USD')}; unsub=${fmt(b.email.unsubscribeRate)}`
    );
  }
  if (b.website) {
    lines.push('');
    lines.push('WEBSITE');
    lines.push(
      `  sessions=${fmt(b.website.sessions)}; cvr=${fmt(b.website.conversionRate)}; bounce=${fmt(b.website.bounceRate)}`
    );
    lines.push(
      `  top pages: ${b.website.topLandingPages
        .slice(0, 3)
        .map((p) => `${p.url}(${p.sessions})`)
        .join(', ')}`
    );
  }
  if (b.pipeline) {
    ...
    lines.push(
      `  opps=${fmt(b.pipeline.opportunitiesCreated)}; win rate=${fmt(b.pipeline.winRate)}; ACV=${fmt(b.pipeline.avgDealSize, ' USD')}`
    );
  ...
  if (b.reviews) {
    ...
    lines.push('REVIEWS');
    lines.push(
      `  new=${fmt(b.reviews.newReviews)}; avg rating=${fmt(b.reviews.avgRating, ' stars')}; response rate=${fmt(b.reviews.responseRate)}`
    );
  ...
  if (b.gbp) {
    ...
    lines.push(
      `  views=${fmt(b.gbp.views)}; actions=${fmt(b.gbp.actions)}; directions=${fmt(b.gbp.directionRequests)}; calls=${fmt(b.gbp.calls)}`
    );
  }
  lines.push('');
  ...
  if (b.competitive.seoRankShifts.length) {
    lines.push(
      `  SEO shifts: ${b.competitive.seoRankShifts
        .slice(0, 5)
        .map((s) => `${s.keyword} ${s.fromRank}→${s.toRank}`)
        .join(', ')}`
    );
  }
  lines.push('');
