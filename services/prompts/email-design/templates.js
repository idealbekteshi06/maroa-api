...
function header({ business, color, eyebrow }) {
  ...
  const logo = business?.logo_url
    ? `<img src="${escapeAttr(business.logo_url)}" alt="${name}" width="40" height="40" style="display:block;border-radius:8px;border:0;"/>`
    : '';
...
function scorecard({ business, marketProfile, scorecardData, commentary }) {
  ...
  const dailySparkline =
    Array.isArray(scorecardData?.roas_daily_7d) && scorecardData.roas_daily_7d.length >= 2
  ...
  const campaignsRanked = Array.isArray(scorecardData?.campaigns_ranked)
    ? scorecardData.campaigns_ranked.slice(0, 3)
    : [];
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
${commentary.top_actions
  .map(
    (a, i) => `<tr><td valign="top" style="padding:8px 0;border-bottom:1px solid #F3F4F6;">
...
const LABELS = {
  en: {
    ...
    top_campaigns_by_roas: 'Top campaigns by ROAS',
    next_actions: 'Top actions for next week',
  ...
  sq: {
    ...
    avg_roas: 'ROAS Mes.',
    vs_prev_week: 'vs jav. prej.',
    last_7_days: '7 ditët e fundit',
    top_campaigns_by_roas: 'Fushatat kryesore',
    next_actions: 'Veprimet për javën tjetër',
    weekly_scorecard: 'Raporti Javor',
    audit_score: 'Pikët',
    ad_audit: 'Auditi i Reklamave',
  },
  es: {
    spend: 'Gasto',
    ...
    top_campaigns_by_roas: 'Mejores campañas por ROAS',
    next_actions: 'Acciones para la próxima semana',
    weekly_scorecard: 'Reporte Semanal',
    audit_score: 'Puntuación',
    ad_audit: 'Auditoría de Anuncios',
  },
  fr: {
    spend: 'Dépenses',
    conversions: 'Conversions',
    ...
    top_campaigns_by_roas: 'Meilleures campagnes par ROAS',
    next_actions: 'Actions pour la semaine prochaine',
    weekly_scorecard: 'Rapport Hebdomadaire',
    audit_score: 'Score',
    ad_audit: 'Audit des Annonces',
  },
  de: {
    spend: 'Ausgaben',
    ...
    top_campaigns_by_roas: 'Top Kampagnen nach ROAS',
    next_actions: 'Aktionen für nächste Woche',
    weekly_scorecard: 'Wochenbericht',
    audit_score: 'Audit Score',
    ad_audit: 'Anzeigen-Audit',
  },
  it: {
    spend: 'Spesa',
    conversions: 'Conversioni',
    avg_roas: 'ROAS Med.',
    vs_prev_week: 'vs sett. prec.',
    last_7_days: 'ultimi 7 giorni',
    top_campaigns_by_roas: 'Migliori campagne per ROAS',
    next_actions: 'Azioni per la prossima settimana',
    weekly_scorecard: 'Riepilogo Settimanale',
    audit_score: 'Punteggio',
    ad_audit: 'Audit Annunci',
  },
  pt: {
    spend: 'Gasto',
    ...
    top_campaigns_by_roas: 'Principais campanhas por ROAS',
    next_actions: 'Ações para a próxima semana',
    weekly_scorecard: 'Relatório Semanal',
    audit_score: 'Pontuação',
    ad_audit: 'Auditoria de Anúncios',
  },
};
...
function localizeEyebrow(lang, kind = 'weekly') {
  if (kind === 'ad_audit') {
    return (
      {
        en: 'Ad Audit',
        sq: 'Auditi i Reklamave',
        es: 'Auditoría de Anuncios',
        fr: 'Audit des Annonces',
        de: 'Anzeigen-Audit',
        it: 'Audit Annunci',
        pt: 'Auditoria de Anúncios',
      }[lang] || 'Ad Audit'
    );
  }
  return (
