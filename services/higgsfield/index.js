...
module.exports = function createHiggsfieldService(deps) {
  ...
  const higgsfieldSdk = createSdkWrapper({
    apiBase: HIGGSFIELD_API_BASE,
    keyId: HIGGSFIELD_API_KEY_ID,
    keySecret: HIGGSFIELD_API_KEY_SECRET,
    pollIntervalMs: POLL_INTERVAL_MS,
    maxPollTimeMs: VIDEO_JOB_TIMEOUT_MS,
    timeoutMs: 120000,
  });

  function logRouter(msg) {
    logger.warn('higgsfield', null, msg);
  }
  ...
  async function submitSoulAndWait(payload, pathOverride) {
    const path = pathOverride || PATH_SOUL;
    const postUrl = higgsfieldUrl(path);
    console.error('[higgsfield:submitSoulAndWait] SDK subscribe', postUrl);
    ...
    try {
      const { url } = await higgsfieldSdk.subscribeAndWait(path, payload, 'image');
      return url;
    } catch (err) {
      console.error('[higgsfield:submitSoulAndWait] SDK failed:', err && err.message);
  ...
  async function submitVideoAndWait(path, payload) {
    const postUrl = higgsfieldUrl(path);
    console.error('[higgsfield:submitVideoAndWait] SDK subscribe', postUrl);
    ...
    try {
      const { url } = await higgsfieldSdk.subscribeAndWait(path, payload, 'video');
      return url;
    } catch (err) {
      console.error('[higgsfield:submitVideoAndWait] SDK failed:', err && err.message);
      console.error(err && err.stack);
      throw err;
    }
  }
  ...
  async function generateWithModel(modelId, payload, kind = 'image') {
    const resolved = resolveModelSlug(modelId, logRouter);
    const path = pathForModel(resolved.canonical);
    if (kind === 'video') return submitVideoAndWait(path, payload);
    return submitSoulAndWait(payload, path);
  }

  async function recordGenerationCost(businessId, modelSlug, meta = {}) {
    const est = estimateModelCost(modelSlug);
    await logHiggsfieldGenerationCost(sbPost, {
      businessId,
      model: modelSlug,
      credits_used: est.credits,
      cost_usd: est.cost_usd,
      skill: meta.skill || 'higgsfield_generation',
    });
    return est;
  }

  /**
   * WF10 adapter — image generation with content_type routing + camera presets.
   */
  async function generateImage(args = {}) {
    const routed = routeModelForContentType(args.content_type, args.model, logRouter);
    const preset = getCameraPreset(args.preset || args.camera_preset);
    const path = pathForModel(routed.model);
    let payload = applyCameraPresetToPayload(
      {
        prompt: hf.killSlop(args.prompt || ''),
        aspect_ratio: args.aspect_ratio || preset.aspect_ratio || '1:1',
    ...
    return {
      ...
      model_slug: routed.model_slug,
      camera_preset: preset.id,
      credits_used: est.credits,
  ...
  /**
   * WF10 adapter — video generation with smart routing + camera presets.
   */
  async function generateVideo(args = {}) {
    const routed = routeModelForContentType(args.content_type, args.model, logRouter);
    const preset = getCameraPreset(args.preset || args.camera_preset);
    const path = pathForModel(routed.model);
    const motionBase = args.motionPrompt || args.motion_prompt || args.prompt || '';
    const motion = motionPromptFromPreset(preset.id, motionBase);
    let payload = applyCameraPresetToPayload(
      {
        prompt: hf.killSlop(motion),
        aspect_ratio: args.aspectRatio || args.aspect_ratio || preset.aspect_ratio || '9:16',
        resolution: args.resolution || '720p',
      },
      preset.id
    );
    if (args.sourceImageUrl || args.source_image_url) {
      payload.image_url = args.sourceImageUrl || args.source_image_url;
    }
