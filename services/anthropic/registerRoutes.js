...
function trace(name, fn) {
  return async function tracedHandler(req, res) {
    ...
    const transaction = Sentry?.startTransaction
      ? Sentry.startTransaction({ op: 'http.server', name: `POST ${name}` })
      : null;
    try {
      await fn(req, res);
    } catch (e) {
...
function registerAnthropicRoutes(deps) {
  ...
  app.post(
    '/webhook/anthropic-file-upload',
    limiters.fileUpload,
    trace('/webhook/anthropic-file-upload', async (req, res) => {
      const { businessId, filename, mimeType, base64Content, kind, description, makeDefault } = req.body || {};
      if (!businessId || !filename || !base64Content)
        return apiError(res, 400, 'INVALID_REQUEST', 'businessId + filename + base64Content required');
      ...
      try {
        ...
        if (totalFiles >= limit) {
          return apiError(
            ...
            429,
            'PLAN_LIMIT_REACHED',
            `Plan ${ctx.plan || 'starter'} allows ${limit} files. Current: ${totalFiles}.`
          );
        }
        ...
        if (makeDefault && persisted) {
          ...
          if (newId)
            await sbPatch(
              'anthropic_files',
              `business_id=eq.${businessId}&id=neq.${newId}&kind=eq.${kind || 'brand_guidelines'}`,
              { is_default: false }
            ).catch(() => {});
        }
  ...
  app.get(
    '/webhook/anthropic-files-list',
    limiters.filesList,
    trace('/webhook/anthropic-files-list', async (req, res) => {
  ...
  app.post(
    '/webhook/anthropic-file-delete',
    limiters.fileMutate,
    trace('/webhook/anthropic-file-delete', async (req, res) => {
      const { businessId, fileId } = req.body || {};
      if (!businessId || !fileId) return apiError(res, 400, 'INVALID_REQUEST', 'businessId + fileId required');
      try {
        const rows = await sbGet(
          'anthropic_files',
          `id=eq.${fileId}&business_id=eq.${businessId}&select=anthropic_file_id`
        );
        const row = rows[0];
  ...
  app.post(
    '/webhook/anthropic-file-set-default',
    limiters.fileMutate,
    trace('/webhook/anthropic-file-set-default', async (req, res) => {
      ...
      try {
        ...
        await sbPatch('anthropic_files', `id=eq.${fileId}&business_id=eq.${businessId}`, {
          is_default: true,
  ...
  app.post(
    '/webhook/anthropic-batch-submit',
    limiters.batchSubmit,
    trace('/webhook/anthropic-batch-submit', async (req, res) => {
      const { purpose, requests } = req.body || {};
      if (!Array.isArray(requests) || requests.length === 0) {
        return apiError(res, 400, 'INVALID_REQUEST', 'requests must be a non-empty array');
      }
      if (requests.length > 1000)
        return apiError(res, 400, 'BATCH_TOO_LARGE', 'max 1000 requests per submission via this route');
      ...
      try {
        ...
        for (const bid of businessIds) {
          ...
          const planCheck = await checkMonthlyLimit(
            sbGet,
            ctx.user_id || bid,
            'anthropic_batch',
            ctx.plan,
          ...
          if (!planCheck.allowed) {
            return apiError(
              ...
              429,
              'PLAN_LIMIT_REACHED',
              `Business ${bid} reached monthly batch limit (${planCheck.limit}).`
            );
          }
        }
  ...
  app.get(
    '/webhook/anthropic-batch-status',
    limiters.batchPoll,
    trace('/webhook/anthropic-batch-status', async (req, res) => {
  ...
  app.post(
    '/webhook/anthropic-batch-reconcile',
    limiters.batchPoll,
    trace('/webhook/anthropic-batch-reconcile', async (req, res) => {
  ...
  async function runAnthropicBatchReconcileAll() {
    ...
    return { scanned: inflight.length, reconciled, errors, results };
  }
  internalDispatcher.register('/webhook/anthropic-batch-reconcile-all', () => runAnthropicBatchReconcileAll());

  app.post(
    '/webhook/anthropic-batch-reconcile-all',
    limiters.batchPoll,
    trace('/webhook/anthropic-batch-reconcile-all', async (req, res) => {
      try {
        res.json(await runAnthropicBatchReconcileAll());
      } catch (e) {
  ...
  app.post(
    '/webhook/anthropic-batch-cancel',
    limiters.batchPoll,
    trace('/webhook/anthropic-batch-cancel', async (req, res) => {
      const id = req.body?.anthropicBatchId || req.body?.batchId;
      if (!id) return apiError(res, 400, 'INVALID_REQUEST', 'anthropicBatchId required');
      try {
  ...
  app.post(
    '/webhook/insights-with-citations',
    limiters.insights,
    trace('/webhook/insights-with-citations', async (req, res) => {
      ...
      try {
        ...
        if (useFileIds.length === 0) {
          const defaults = await sbGet(
            'anthropic_files',
            `business_id=eq.${businessId}&is_default=eq.true&select=anthropic_file_id`
          ).catch(() => []);
  ...
  // ─── Memory (public beta) ───────────────────────────────────────────
  app.post(
    '/webhook/memory-ensure-session',
    limiters.memoryWrite,
    trace('/webhook/memory-ensure-session', async (req, res) => {
  ...
  app.post(
    '/webhook/memory-append-fact',
    limiters.memoryWrite,
    trace('/webhook/memory-append-fact', async (req, res) => {
  ...
  app.get(
    '/webhook/memory-get-session',
    limiters.memoryRead,
    trace('/webhook/memory-get-session', async (req, res) => {
  ...
  app.post(
    '/webhook/memory-delete-session',
    limiters.memoryWrite,
    trace('/webhook/memory-delete-session', async (req, res) => {
  ...
  app.post(
    '/webhook/managed-agent-run',
    limiters.agentRun,
    trace('/webhook/managed-agent-run', async (req, res) => {
      ...
      try {
        if (checkOrchestrationIdempotency) {
          const isDup = await checkOrchestrationIdempotency(
            businessId,
            `agent_run:${(message || '').slice(0, 80)}`,
            30000
          );
        ...
        const agent = await managedAgentService.ensureAgent({
          ...
          instructions:
            instructions ||
            `You are Maroa's AI Brain for business ${businessId}. Delegate to internal MCP tools when you need data.`,
          tools: tools || [],
        });
  ...
  app.get(
    '/webhook/managed-agent-poll',
    limiters.agentPoll,
    trace('/webhook/managed-agent-poll', async (req, res) => {
      const sessionId = req.query?.sessionId;
      if (!sessionId) return apiError(res, 400, 'INVALID_REQUEST', 'sessionId required');
      try {
  ...
  app.post(
    '/webhook/managed-agent-cancel',
    limiters.agentRun,
    trace('/webhook/managed-agent-cancel', async (req, res) => {
      const { sessionId } = req.body || {};
      if (!sessionId) return apiError(res, 400, 'INVALID_REQUEST', 'sessionId required');
      try {
