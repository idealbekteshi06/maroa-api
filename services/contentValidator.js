...
function validateGeneratedContent(content, profile, taskType) {
  ...
  const adAreas = Array.isArray(profile?.ad_targeting_area)
    ? profile.ad_targeting_area.map((c) => c.toLowerCase())
    : [];
  const locs = Array.isArray(profile?.physical_locations) ? profile.physical_locations : [];
  const allowedCities = [
    ...adAreas,
    ...locs.map((l) => (l.city || '').toLowerCase()),
    ...locs.map((l) => (l.neighborhood || '').toLowerCase()),
  ].filter(Boolean);
  ...
  if (profile?.primary_language === 'Albanian') {
    const engWords = (
      content.match(/\b(the|and|but|is|are|was|were|have|has|this|that|with|from|your|our|their)\b/gi) || []
    ).length;
