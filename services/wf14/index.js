...
function createWf14(deps) {
  ...
  async function runOptimizer({ businessId, force = false }) {
    ...
    const existing = await sbGet(
      'budget_optimizer_runs',
      `business_id=eq.${businessId}&month_start=eq.${monthStartStr}&select=id`
    ).catch(() => []);
    ...
    const raw = await callClaude(user, 'claude-opus-4-8', 3000, { system, businessId, returnRaw: true });
  ...
  async function getLatest(businessId) {
    const rows = await sbGet(
      'budget_optimizer_runs',
      `business_id=eq.${businessId}&order=month_start.desc&limit=1&select=*`
    ).catch(() => []);
    return rows[0] || null;
  }
