...
function getKosovoAlbaniaHolidays(date) {
  const holidays = {
    '01-01': 'Viti i Ri',
    '01-07': 'Krishtlindjet Ortodokse',
    '02-17': 'Dita e Pavarësisë së Kosovës',
    '03-08': 'Dita e Gruas',
    '04-09': 'Dita e Kushtetutës',
    '05-01': 'Dita e Punës',
    '05-09': 'Dita e Evropës',
    '06-01': 'Dita e Fëmijëve',
    '06-12': 'Dita e Paqes',
    '11-28': 'Dita e Flamurit',
    '11-29': 'Dita e Çlirimit',
    '12-25': 'Krishtlindjet',
    '12-31': 'Nata e Vitit të Ri',
  };
  const upcoming = [];
...
function getAudiencePsychology(businessType, gender, ageMin) {
  ...
  if (type.includes('fitness') || type.includes('gym'))
    return 'transformation desire, achievement motivation, social comparison, before/after framing';
  if (type.includes('restaurant') || type.includes('food') || type.includes('cafe'))
    return 'sensory language, social experience, FOMO, tradition and comfort';
  if (type.includes('beauty') || type.includes('salon'))
    return 'confidence boost, self-image improvement, luxury aspiration, visible results';
  if (type.includes('retail') || type.includes('shop'))
    return 'value perception, exclusivity, trend awareness, social proof';
  if (type.includes('medical') || type.includes('health') || type.includes('dental'))
    return 'trust and safety, expert credibility, relief from worry, family protection';
  if (type.includes('real estate') || type.includes('property'))
    return 'aspiration, security, investment thinking, lifestyle upgrade';
  if (type.includes('education') || type.includes('tutor'))
    return 'growth mindset, future potential, parental pride, career advancement';
...
function buildMasterPrompt(p, taskType = 'general') {
  ...
  const locationStr =
    locs.map((l) => (l.neighborhood ? `${l.neighborhood}, ${l.city}` : l.city)).join(' | ') || primaryCity;
  ...
  let countryName = '';
  try {
    const ci = require('./countryIntelligence');
    const cd = ci.getCountryIntelligence(ci.detectCountry(p));
    countryName = cd.name;
    intlCtx = ci.buildInternationalContext(p);
  } catch (e) {
...
${
  prods.length > 0
    ? prods
        .map(
          (pr) =>
            `- ${pr.name}${pr.price ? ' (' + pr.price + ')' : ''}: ${pr.description || ''}${pr.is_bestseller ? ' ★ BESTSELLER' : ''}${pr.is_most_profitable ? ' 💰 MOST PROFITABLE' : ''}`
        )
        .join('\n')
    : '- No products listed'
}
 ...
 */
async function buildMasterPromptWithSkills(
  profile,
  taskType,
  getEmbedding,
  pineconeQuery,
  buildIntelligenceCtx,
  getMemoryCtx,
  extraContext = ''
) {
  ...
  try {
    ...
    let skills = [];
    try {
      skills = await getRelevantSkills(
        getEmbedding,
        pineconeQuery,
        taskType,
    ...
    if (!skills.length) {
      ...
      const taskTypeMap = {
        social_post: ['social_content_strategy', 'copywriting_principles'],
        paid_ad: ['ad_creative_frameworks', 'paid_ads_strategy'],
        email: ['email_sequence_strategy', 'copywriting_principles'],
        content_calendar: ['content_strategy', 'social_content_strategy'],
        sales_pitch: ['copywriting_principles', 'marketing_psychology'],
        lead_magnet: ['content_strategy', 'marketing_psychology'],
        launch: ['launch_strategy', 'content_strategy'],
        general: ['marketing_psychology', 'copywriting_principles'],
      };
      const relevantIds = taskTypeMap[taskType] || taskTypeMap.general;
      skills = allSkills
        .filter((s) => relevantIds.includes(s.id))
        .map((s) => ({ name: s.name, content: s.content.slice(0, 1500), score: 0.9 }));
    }
