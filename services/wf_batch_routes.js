...
function registerBatchRoutes({ app, wf5, wf6, wf7, wf8, wf9, wf10, wf12, wf14, apiError, logger }) {
  ...
  app.post('/webhook/wf5-run-analysis', async (req, res) => {
    ...
    try {
      res.json(await wf5.runAnalysis({ businessId, force: !!force }));
    } catch (e) {
      apiError(res, 500, 'WF5_FAILED', e.message);
    }
  });
  async function wf5latest(req, res) {
    ...
    try {
      res.json((await wf5.getLatest(businessId)) || null);
    } catch (e) {
      apiError(res, 500, 'WF5_LATEST_FAILED', e.message);
    }
  }
  ...
  app.post('/webhook/wf6-run-audit', async (req, res) => {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json(await wf6.runAudit({ businessId, auditInput }));
    } catch (e) {
      apiError(res, 500, 'WF6_AUDIT_FAILED', e.message);
    }
  });
  app.post('/webhook/wf6-generate-schema', async (req, res) => {
    ...
    try {
      res.json(await wf6.generateSchema({ businessId, page }));
    } catch (e) {
      apiError(res, 500, 'WF6_SCHEMA_FAILED', e.message);
    }
  });
  async function wf6latest(req, res) {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json((await wf6.getLatestAudit(businessId)) || null);
    } catch (e) {
      apiError(res, 500, 'WF6_LATEST_FAILED', e.message);
    }
  }
  ...
  app.post('/webhook/wf7-segment-create', async (req, res) => {
    ...
    try {
      res.json(await wf7.createSegment(body));
    } catch (e) {
      apiError(res, 500, 'WF7_SEG_FAILED', e.message);
    }
  });
  app.post('/webhook/wf7-design-sequence', async (req, res) => {
    ...
    try {
      res.json(await wf7.designSequence({ businessId, segmentId }));
    } catch (e) {
      apiError(res, 500, 'WF7_DESIGN_FAILED', e.message);
    }
  });
  app.post('/webhook/wf7-enroll', async (req, res) => {
    ...
    try {
      res.json(await wf7.enrollContact({ businessId, sequenceId, contactId }));
    } catch (e) {
      apiError(res, 500, 'WF7_ENROLL_FAILED', e.message);
    }
  });
  app.post('/webhook/wf7-dispatch-due', async (req, res) => {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'businessId required');
    try {
      res.json(await wf7.dispatchDue({ businessId }));
    } catch (e) {
      apiError(res, 500, 'WF7_DISPATCH_FAILED', e.message);
    }
  });
  ...
  app.post('/webhook/wf8-generate-report', async (req, res) => {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'businessId required');
    try {
      res.json(await wf8.generateInsightReport({ businessId }));
    } catch (e) {
      apiError(res, 500, 'WF8_FAILED', e.message);
    }
  });
  async function wf8latest(req, res) {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json((await wf8.getLatestReport(businessId)) || null);
    } catch (e) {
      apiError(res, 500, 'WF8_LATEST_FAILED', e.message);
    }
  }
  ...
  app.post('/webhook/wf9-draft-reply', async (req, res) => {
    ...
    if (!businessId || !threadId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json(await wf9.draftReply({ businessId, threadId, triage }));
    } catch (e) {
      apiError(res, 500, 'WF9_DRAFT_FAILED', e.message);
    }
  });
  async function wf9list(req, res) {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json(await wf9.listThreads({ businessId, status: req.query?.status, urgency: req.query?.urgency }));
    } catch (e) {
      apiError(res, 500, 'WF9_LIST_FAILED', e.message);
    }
  }
  ...
  app.post('/webhook/wf10-create-job', async (req, res) => {
    ...
    try {
      res.json(await wf10.createStudioJob({ businessId, request }));
    } catch (e) {
      apiError(res, 500, 'WF10_CREATE_FAILED', e.message);
    }
  });
  async function wf10get(req, res) {
    ...
    try {
      res.json((await wf10.getJob({ businessId, jobId })) || null);
    } catch (e) {
      apiError(res, 500, 'WF10_GET_FAILED', e.message);
    }
  }
  ...
  async function wf10list(req, res) {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json(await wf10.listJobs({ businessId, status: req.query?.status }));
    } catch (e) {
      apiError(res, 500, 'WF10_LIST_FAILED', e.message);
    }
  }
  ...
  app.post('/webhook/wf12-plan-launch', async (req, res) => {
    ...
    if (!businessId || !request) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json(await wf12.planLaunch({ businessId, request }));
    } catch (e) {
      apiError(res, 500, 'WF12_PLAN_FAILED', e.message);
    }
  });
  app.post('/webhook/wf12-activity-update', async (req, res) => {
    ...
    try {
      res.json(await wf12.updateActivityStatus({ businessId, activityId, status }));
    } catch (e) {
      apiError(res, 500, 'WF12_UPDATE_FAILED', e.message);
    }
  });
  async function wf12list(req, res) {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json(await wf12.listLaunches(businessId));
    } catch (e) {
      apiError(res, 500, 'WF12_LIST_FAILED', e.message);
    }
  }
  ...
  async function wf12get(req, res) {
    ...
    try {
      res.json(await wf12.getLaunchDetail({ businessId, launchId }));
    } catch (e) {
      apiError(res, 500, 'WF12_GET_FAILED', e.message);
    }
  }
  ...
  app.post('/webhook/wf14-run', async (req, res) => {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json(await wf14.runOptimizer({ businessId, force: !!force }));
    } catch (e) {
      apiError(res, 500, 'WF14_FAILED', e.message);
    }
  });
  async function wf14latest(req, res) {
    ...
    if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'required');
    try {
      res.json((await wf14.getLatest(businessId)) || null);
    } catch (e) {
      apiError(res, 500, 'WF14_LATEST_FAILED', e.message);
    }
  }
