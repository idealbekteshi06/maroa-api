...
function registerCreativeRoutes({
  app,
  hfService,
  sbGet,
  sbPost,
  ...
  app.post(
    '/webhook/develop-creative-concept',
    limiters.develop,
    trace('/webhook/develop-creative-concept', async (req, res) => {
      const { businessId, businessGoal, contentGoal, ideaLevel, abVariant } = req.body || {};
      if (!businessId || !contentGoal)
        return apiError(res, 400, 'INVALID_REQUEST', 'businessId + contentGoal required');
      ...
      try {
        ...
        if (checkOrchestrationIdempotency) {
          const isDup = await checkOrchestrationIdempotency(
            businessId,
            `creative_concept:${(contentGoal || '').slice(0, 50)}`,
            60000
          );
        ...
        if (!planCheck.allowed) {
          return apiError(
            ...
            429,
            'PLAN_LIMIT_REACHED',
            `Monthly creative-director limit reached (${planCheck.limit}). Current: ${planCheck.count}.`
          );
        }
        ...
        const variant =
          abVariant || pickVariant(`${businessId}|${contentGoal}|${new Date().toISOString().slice(0, 10)}`);
  ...
  app.post(
    '/webhook/creative-concept-decision',
    limiters.decide,
    trace('/webhook/creative-concept-decision', async (req, res) => {
      ...
      if (!['approve', 'reject', 'use'].includes(decision))
        return apiError(res, 400, 'INVALID_REQUEST', 'decision must be approve|reject|use');
  ...
  app.get(
    ...
    limiters.list,
    trace('/webhook/creative-concepts-list', async (req, res) => {
  ...
  app.post(
    '/webhook/character-create',
    limiters.charCreate,
    trace('/webhook/character-create', async (req, res) => {
      ...
      const urls = Array.isArray(sourceImageUrls)
        ? sourceImageUrls.filter((u) => typeof u === 'string' && u.startsWith('http'))
        : [];
      ...
      try {
        ...
        const limit =
          PLAN_CHARACTER_LIMIT[(ctx.business.plan || 'starter').toLowerCase()] ?? PLAN_CHARACTER_LIMIT.starter;
        if (totalCharacters >= limit) {
          return apiError(
            res,
            429,
            'PLAN_LIMIT_REACHED',
            `Plan ${ctx.business.plan || 'starter'} allows max ${limit} characters. Current: ${totalCharacters}.`
          );
        }
        ...
        if (makeDefault && characterId) {
          await sbPatch('business_characters', `business_id=eq.${businessId}&id=neq.${characterId}`, {
            is_default: false,
          }).catch(() => {});
        }
        ...
        res.json({
          ...
          creditCost: 40,
          planUsage: { count: totalCharacters + 1, limit },
        });
      } catch (e) {
  ...
  app.post(
    '/webhook/character-train',
    limiters.charTrain,
    trace('/webhook/character-train', async (req, res) => {
      const { businessId, characterId } = req.body || {};
      if (!businessId || !characterId)
        return apiError(res, 400, 'INVALID_REQUEST', 'businessId + characterId required');
      try {
        if (checkOrchestrationIdempotency) {
        ...
        if (character.training_status === 'training')
          return apiError(res, 409, 'ALREADY_TRAINING', 'character already training');
        ...
        hfService
          ...
          .catch(async (e) => {
            Sentry?.captureException?.(e, {
              tags: { workflow: 'soul_id', business_id: businessId, character_id: characterId },
            });
  ...
  app.get(
    ...
    limiters.list,
    trace('/webhook/characters-list', async (req, res) => {
      const businessId = req.query?.business_id || req.query?.businessId;
      if (!businessId) return apiError(res, 400, 'INVALID_REQUEST', 'business_id required');
      try {
        const rows = await sbGet(
  ...
  app.post(
    '/webhook/character-set-default',
    limiters.charDefault,
    trace('/webhook/character-set-default', async (req, res) => {
      const { businessId, characterId } = req.body || {};
      if (!businessId || !characterId)
        return apiError(res, 400, 'INVALID_REQUEST', 'businessId + characterId required');
      try {
        await sbPatch('business_characters', `business_id=eq.${businessId}`, { is_default: false });
        await sbPatch('business_characters', `id=eq.${characterId}&business_id=eq.${businessId}`, {
          is_default: true,
          updated_at: new Date().toISOString(),
        });
        res.json({ ok: true });
  ...
  app.post(
    '/webhook/vet-customer-asset',
    limiters.vet,
    trace('/webhook/vet-customer-asset', async (req, res) => {
      ...
      try {
        ...
        if (checkOrchestrationIdempotency) {
          const isDup = await checkOrchestrationIdempotency(
            businessId,
            `vet:${imageUrl.slice(0, 80)}|${(contentTheme || '').slice(0, 30)}`,
            60000
          );
        ...
        if (!planCheck.allowed) {
          return apiError(
            ...
            429,
            'PLAN_LIMIT_REACHED',
            `Monthly vetter limit reached (${planCheck.limit}). Current: ${planCheck.count}.`
          );
        }
  ...
  app.post(
    '/webhook/vet-customer-asset-batch',
    limiters.vetBatch,
    trace('/webhook/vet-customer-asset-batch', async (req, res) => {
      ...
      try {
        const ctx = await getBrandDNAFor(businessId, sbGet);
        if (!ctx) return apiError(res, 404, 'BUSINESS_NOT_FOUND', `Business not found: ${businessId}`);
        const grouped = await hfService.vetCustomerAssetBatch(imageUrls, ctx.brandDNA, {
  ...
  app.post(
    '/webhook/smart-process-asset',
    limiters.smart,
    trace('/webhook/smart-process-asset', async (req, res) => {
      const { businessId, imageUrl, contentTheme } = req.body || {};
      if (!businessId || !imageUrl) return apiError(res, 400, 'INVALID_REQUEST', 'businessId + imageUrl required');
      try {
        if (checkOrchestrationIdempotency) {
        ...
        const result = await hfService.smartProcessAsset(imageUrl, ctx.brandDNA, {
          contentTheme: contentTheme || '',
          userId,
        });
        ...
        try {
          ...
          /* audit write-back is best-effort */
        }
