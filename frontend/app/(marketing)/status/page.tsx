...
const STATUS_STYLES = {
  operational: { label: 'Operational', dot: 'bg-green-500', text: 'text-green-700', bg: 'bg-green-50' },
  degraded: { label: 'Degraded', dot: 'bg-amber-500', text: 'text-amber-700', bg: 'bg-amber-50' },
  outage: { label: 'Outage', dot: 'bg-red-500', text: 'text-red-700', bg: 'bg-red-50' },
} as const;
  ...
  return (
    <section className="container pt-20 sm:pt-28 pb-32">
      <div className="max-w-3xl mx-auto">
        ...
        <div className={`rounded-2xl p-5 mb-12 flex items-center gap-3 ${allOk ? 'bg-green-50 border border-green-200/60' : 'bg-amber-50 border border-amber-200/60'}`}>
          <span className={`h-3 w-3 rounded-full ${allOk ? 'bg-green-500 animate-pulse' : 'bg-amber-500 animate-pulse'}`} aria-hidden="true" />
          <p className={`text-sm font-medium ${allOk ? 'text-green-700' : 'text-amber-700'}`}>
            {allOk
        ...
        <h2 className="text-2xl font-semibold text-ink-700 mb-6">Services</h2>
        <div className="space-y-2">
          {SERVICES.map((s) => {
            ...
            return (
              <div
                key={s.name}
                className="flex items-center justify-between p-4 rounded-xl bg-white border border-ink-200/60"
              >
                <span className="font-medium text-ink-700">{s.name}</span>
                <span className={`text-xs font-medium px-3 py-1 rounded-full inline-flex items-center gap-2 ${style.bg} ${style.text}`}>
        ...
        <h2 className="text-2xl font-semibold text-ink-700 mt-16 mb-6">Recent incidents</h2>
        <div className="rounded-xl border border-ink-200/60 bg-white p-8 text-center">
          <p className="text-ink-400">No incidents in the past 30 days.</p>
        </div>

        <h2 className="text-2xl font-semibold text-ink-700 mt-16 mb-4">Stay informed</h2>
