  ...
  return (
    <section className="container pt-20 sm:pt-28 pb-32">
      <div className="max-w-3xl mx-auto text-center mb-16">
        ...
        <h1 className="text-display-lg text-ink-700">Talk to a human.</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
        {CONTACTS.map((c) => (
          <a
            key={c.label}
            href={`mailto:${c.email}`}
            className="block rounded-2xl border border-ink-200/60 bg-white p-6 hover:border-accent-500 hover:shadow-card transition-all"
          >
            <div className="h-10 w-10 rounded-xl bg-ink-100 flex items-center justify-center mb-4">
              <c.icon className="h-5 w-5 text-ink-700" aria-hidden="true" />
            </div>
            <p className="text-sm font-medium text-ink-400 mb-1">{c.label}</p>
            <p className="text-lg font-semibold text-ink-700">{c.email}</p>
            <p className="mt-2 text-sm text-ink-400 leading-relaxed">{c.description}</p>
          </a>
        ))}
      </div>

      <div className="max-w-3xl mx-auto mt-16 prose prose-lg text-ink-700 leading-relaxed">
        <h2 className="text-2xl font-semibold text-ink-700 mb-4">Office hours</h2>
        <p>
          ...
          <code className="bg-ink-100 px-2 py-1 rounded text-sm">[URGENT]</code> and we&apos;ll
          page on-call.
        </p>

        <h2 className="text-2xl font-semibold text-ink-700 mt-12 mb-4">Press + partnerships</h2>
        <p>
          Email <a href="mailto:press@maroa.ai" className="text-accent-500 hover:underline">press@maroa.ai</a>.
        </p>

        <h2 className="text-2xl font-semibold text-ink-700 mt-12 mb-4">Legal entity</h2>
        <p className="text-ink-400">
          Maroa AI, Inc. · Delaware C-corp · EIN on request via{' '}
          <a href="mailto:legal@maroa.ai" className="text-accent-500 hover:underline">legal@maroa.ai</a>.
        </p>
      </div>
    </section>
  );
