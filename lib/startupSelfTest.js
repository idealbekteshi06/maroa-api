...
async function probeAnthropic({ logger }) {
  ...
  try {
    ...
    if (!ok) {
      logger?.warn?.('startup-self-test', null, 'anthropic probe soft-fail (non-blocking)', {
        status: r.status,
      });
    }
    return { ok, status: r.status, soft: !ok };
  } catch (e) {
    logger?.warn?.('startup-self-test', null, 'anthropic probe soft-fail (non-blocking)', { error: e.message });
    return { ok: false, error: e.message, soft: true };
  }
}
...
async function runStartupSelfTest({ sbGet, logger }) {
  ...
  const softWarnings = Object.entries(results)
    .filter(([k, r]) => !r.ok && k !== 'supabase' && k !== 'env')
    .map(([k]) => k);
  // Boot-critical: required env + Supabase only. Anthropic/Higgsfield-style deps are soft.
  const bootCriticalOk = results.env.ok && results.supabase.ok;
  const summary = {
    passed,
    total,
    healthy: bootCriticalOk,
    boot_critical_ok: bootCriticalOk,
    soft_warnings: softWarnings,
    duration_ms: Date.now() - started,
    timestamp: new Date().toISOString(),
    results,
  };
  const level = bootCriticalOk ? 'info' : 'warn';
  logger?.[level]?.('startup-self-test', null, `Boot self-test ${passed}/${total} probes passed (critical=${bootCriticalOk})`, summary);
