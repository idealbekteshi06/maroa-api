...
function makeDecisionLogger(deps = {}) {
  ...
  async function proposeDecision(spec = {}) {
    ...
    const row = {
      ...
      expected_upside_value:
        typeof expectedUpside?.value === 'number' ? expectedUpside.value : null,
      risk_text: risk || null,
      cost_usd: typeof costUsd === 'number' ? Number(costUsd.toFixed(2)) : 0,
      manipulation_risk:
        typeof manipulationRisk === 'number' ? manipulationRisk : null,
      auto_safe_band: band,
  ...
  async function recentDecisions(businessId, { agentName, limit = 50 } = {}) {
    ...
    if (!(await isHealthy())) return [];
    let filter =
      `business_id=eq.${_encode(businessId)}&order=created_at.desc` +
  ...
  async function reject(id, userId, reason) {
    ...
    const note = trimmed
      ? `rejected by user ${userId}: ${trimmed}`
