...
async function buildKpiHistory({ sbGet, businessIds, currentSummary, now = Date.now() }) {
  ...
  const [creativeRows, experimentRows, approvalRows, refusalRows, clientRows] = await Promise.all([
    tryGet(
      'creative_assets',
      `${bizIn}&select=id,business_id,created_at&order=created_at.asc&limit=2000`,
    ),
    tryGet(
      'experiments',
      `${bizIn}&select=id,status,started_at,ended_at&limit=2000`,
    ),
    tryGet(
      'client_approvals',
      `${bizIn}&select=id,status,created_at,expires_at&order=created_at.asc&limit=2000`,
    ),
    tryGet(
      'decision_logs',
  ...
  const series = { active_clients, creatives_total, experiments_running, pending_approvals, refusals_7d };
...
module.exports = { buildKpiHistory, weekDeltaPct, trendDir, _internals: { dayEnds, cumulativeFromCreatedAt, perDayFromCreatedAt } };
