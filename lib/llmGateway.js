...
async function enforceLLMBudget({
  ...
  if (!skipCostCap && sbGet) {
    ...
    if (!monthly.allowed) {
      throw new LLMBudgetExceededError(
        monthly.reason || 'Monthly AI spend cap reached',
        'AI_COST_CAP_EXCEEDED',
        402
      );
    }
    return { allowed: true, maxTokensPerCall: 4000, plan: monthly.plan, used_usd: monthly.used_usd, cap_usd: monthly.cap_usd };
  }
