 ...
 */
const BENCHMARK_INDUSTRY_SLUGS = new Set([
  'dental',
  'restaurant',
  'retail',
  'fitness',
  ...
  'ecommerce',
  'cafe',
  // 2026-07 expansion — calibrated in lib/industryBenchmarks + migration 101
  'home_services',
  'healthcare',
  'automotive',
  'saas',
...
/**
 * Map free-text business.industry to the closest seeded industry_benchmarks slug.
 * Returns 'global' when no vertical-specific match exists.
 */
function resolveBenchmarkIndustry(industry) {
  ...
  if (BENCHMARK_INDUSTRY_SLUGS.has(slug) && slug !== 'global') return slug;
  ...
  if (/\b(online[_\s-]?store|ecommerce|e[\s_-]?commerce|webshop|shopify|dropship)\b/.test(text)) {
    return 'ecommerce';
  }
  if (/\b(dentist|orthodont|dental|oral)\b/.test(text)) return 'dental';
  if (/\bclinic\b/.test(text) && !/\b(vet|veterinar|medical|healthcare|hospital)\b/.test(text)) {
    return 'dental';
  }
  if (/\b(attorney|lawyer|law_firm|legal|accountant|accounting|cpa|notary)\b/.test(text)) {
    return 'legal';
  }
  if (/\b(realtor|real[\s_-]?estate|realty|property|mortgage|brokerage)\b/.test(text)) {
    return 'real_estate';
  }
  if (/\b(hair|nail|spa|salon|beauty|cosmetic|esthetician|barber|makeup)\b/.test(text)) {
    return 'beauty';
  }
  if (/\b(gym|fitness|yoga|pilates|crossfit|personal_trainer|workout)\b/.test(text)) {
    return 'fitness';
  }
  if (/\b(clothing|fashion|apparel|accessories|boutique|retail|merch)\b/.test(text)) {
    return 'retail';
  }
  if (/\b(bakery|patisserie|cafe|coffee|espresso|roastery)\b/.test(text)) return 'cafe';
  if (/\b(bar|pub|bistro|brewery|tavern|restaurant|dining|kitchen|food_truck|catering)\b/.test(text)) {
    return 'restaurant';
  }

  for (const ind of BENCHMARK_INDUSTRY_SLUGS) {
    if (ind === 'global') continue;
    if (slug.includes(ind) || text.includes(ind.replace(/_/g, ' '))) return ind;
  }

  return 'global';
}

/** @deprecated alias — use resolveBenchmarkIndustry */
function normalizeIndustrySlug(industry) {
  return resolveBenchmarkIndustry(industry);
}

/**
 * Pull public benchmark row for an industry (GLOBAL region).
 * Resolves fuzzy industry labels, then falls back to the GLOBAL averages row.
 */
async function fetchIndustryBenchmarks({ sbGet, industry, region = 'GLOBAL' }) {
  if (!sbGet) return null;
  const resolved = resolveBenchmarkIndustry(industry);

  async function loadRow(industryKey) {
    try {
      const rows = await sbGet(
        'industry_benchmarks',
        `industry=eq.${encodeURIComponent(industryKey)}&region=eq.${encodeURIComponent(region)}&select=*&limit=1`
      );
  ...
  let row = await loadRow(resolved);
  let usedFallback = false;
  if (!row && resolved !== 'global') {
    row = await loadRow('global');
    usedFallback = true;
  }
  if (!row && resolved === 'global') {
    row = await loadRow('global');
  }
  // Static calibration fallback (2026-07): the DB table shipped unseeded, so
  // benchmark lookups silently returned null and audits ran benchmark-blind.
  // lib/industryBenchmarks mirrors migration 101's seed values.
  if (!row) {
    try {
      const { staticBenchmarkRow } = require('./industryBenchmarks');
      row = staticBenchmarkRow(resolved) || staticBenchmarkRow('global');
      usedFallback = usedFallback || (row && row.industry !== resolved);
    } catch {
      row = null;
    }
  }
  ...
  return {
    ...row,
    _benchmark_resolved_industry: resolved,
    _benchmark_source_industry: String(industry || '').trim() || null,
    _benchmark_used_global_fallback: usedFallback || resolved === 'global',
  };
}
...
function formatBenchmarkComparison(benchmark, metrics = {}) {
  ...
  if (benchmark.meta_avg_ctr != null && metrics.ctr != null) {
    lines.push(`Meta CTR: industry avg ${formatPct(benchmark.meta_avg_ctr)} — you are at ${formatPct(metrics.ctr)}`);
...
/**
 * Competitor ads currently running (2026-07): competitor-watch snapshots the
 * Meta Ad Library every 4h into competitor_signals; this surfaces the
 * longest-running (= proven) ads as structure references for ad/social copy.
 * Longevity is the honest public winner signal — losers get killed in days.
 */
async function fetchCompetitorWinningAds({ sbGet, businessId, limit = 3 }) {
  if (!sbGet || !businessId) return [];
  try {
    const rows = await sbGet(
      'competitor_signals',
      `business_id=eq.${encodeURIComponent(businessId)}&source=eq.meta_ad_library&order=observed_at.desc&limit=6&select=competitor_name,signal_payload,observed_at`
    );
    const now = Date.now();
    const seen = new Set();
    const scored = [];
    for (const row of rows || []) {
      for (const ad of row?.signal_payload?.ads_snapshot || []) {
        const key = ad?.id || `${row.competitor_name}:${(ad?.headline || ad?.text || '').slice(0, 40)}`;
        ...
        const days = start ? Math.max(0, Math.round(((stopRaw || now) - start) / 86400000)) : 0;
        const text = String(ad?.text || ad?.headline || '').trim();
        if (!text) continue;
        scored.push({
          ...
          runtime_days: days,
          is_active: !stopRaw || stopRaw >= now,
        });
      }
    }
    scored.sort((a, b) => b.runtime_days * (b.is_active ? 1.25 : 1) - a.runtime_days * (a.is_active ? 1.25 : 1));
...
async function buildGroundingContext({
  ...
  const wantsCompetitorAds = surface === 'ad_copy' || surface === 'social_post';
  const [performance, voc, cohort, anchorBrandVoice, publishedCount, industryBenchmark, competitorWinningAds] =
    await Promise.all([
      performancePromise,
      ...
      wantsCompetitorAds ? fetchCompetitorWinningAds({ sbGet, businessId }) : [],
    ]);
    ...
    toCitationDocuments({ maxWins = 3, maxLosses = 2, maxCorpus = 3 } = {}) {
      ...
      if (this.brandVoice && (this.brandVoice.tone || this.brandVoice.do_use?.length)) {
        const bvParts = [
          ...
          this.brandVoice.do_not_use?.length ? `Avoid: ${this.brandVoice.do_not_use.slice(0, 12).join(', ')}.` : '',
        ]
          .filter(Boolean)
          .join(' ');
...
function _renderPromptBlock(ctx) {
  ...
  if (ctx.competitorWinningAds && ctx.competitorWinningAds.length) {
    parts.push('## Competitor ads running right now (longevity = proven winners)');
    parts.push(
      'These competitor ads have survived the market — long runtimes mean they pay for themselves. Study the hook mechanic, offer framing, and CTA placement. NEVER copy brand names, slogans, or any verbatim sentence.'
    );
    ctx.competitorWinningAds.forEach((ad, i) => {
      const head = ad.headline ? `"${ad.headline}" — ` : '';
      parts.push(
        `  ${i + 1}. [${ad.competitor}, ${ad.runtime_days}d${ad.is_active ? ', still running' : ''}] ${head}${ad.excerpt}`
      );
    });
    parts.push('');
  }
