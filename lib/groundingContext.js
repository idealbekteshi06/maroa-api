...
async function buildGroundingContext({
  ...
  const [performance, voc, cohort, brandVoice] = await Promise.all([
    performancePromise,
  ...
  const ctx = {
    ...
    isEmpty() {
      return !this.wins.length && !this.losses.length && !this.voc.length && !this.cohort.length && !this.brandVoice;
    },
  };
