...
async function markProcessed({ provider, eventId, sbPost, logger }) {
  ...
  try {
    ...
    // worse than processing once.
    logger?.warn?.('webhook-events', null, 'idempotency DB unreachable — relying on LRU', { error: msg, provider });
    return {
      firstTime: true,
      reason: `idempotency_db_fail_lru_covered: ${msg}`,
      softFail: true,
      source: 'lru_fallback',
    };
  }
