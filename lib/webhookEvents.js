...
const EVENT_ID_EXTRACTORS = {
  ...
  // Shopify dedups on the X-Shopify-Webhook-Id header (unique per delivery),
  // passed explicitly to markProcessed; this body fallback is best-effort only.
  shopify: (body) => body?.id || body?.admin_graphql_api_id,
};
...
/**
 * Evict a (provider, eventId) from the in-process LRU so the provider's NEXT
 * retry is NOT short-circuited as a duplicate. Call this when a handler fails
 * and you want the retry to re-run (otherwise the LRU suppresses re-processing
 * for up to 5 min even though the DB row is 'failed').
 */
function forgetEvent(provider, eventId) {
  if (!provider || !eventId) return;
  _seenEvents.delete(_seenKey(provider, eventId));
}
...
module.exports = { markProcessed, commitProcessed, forgetEvent, middleware, EVENT_ID_EXTRACTORS };
