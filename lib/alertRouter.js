...
function createAlertRouter({ sendEmail, logger, slackWebhookUrl, emailTo, pagerDutyKey } = {}) {
  ...
  function _isRateLimited(key, channel) {
    ...
    // Evict the just-expired entry + opportunistically sweep a few stale ones so
    // the map can't grow unbounded as alert keys diversify (per-business/run keys).
    _rateLimitMap.delete(k);
    let scanned = 0;
    for (const [mk, exp] of _rateLimitMap) {
      if (exp <= now) _rateLimitMap.delete(mk);
      if (++scanned >= 50) break;
    }
    _rateLimitMap.set(k, now + RATE_LIMIT_MS);
    return false;
  }
