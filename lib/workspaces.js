...
function makeWorkspacesService(deps = {}) {
  ...
  async function createWorkspace({ ownerUserId, name, planTier = 'freelancer', slug, whiteLabel, settings, monthlySpendCapUsd }) {
  ...
  async function getWorkspacesForUser(userId) {
    if (!userId) return [];
    const memberships = await _softGet(
      'workspace_members',
      `user_id=eq.${_enc(userId)}&select=workspace_id`
    );
  ...
  async function updateMemberRole({ workspaceId, userId, role, visibleClientIds }) {
    ...
    return _softPatch(
      'workspace_members',
      `workspace_id=eq.${_enc(workspaceId)}&user_id=eq.${_enc(userId)}`,
      patch
    );
  }
  ...
  async function removeMember({ workspaceId, userId }) {
    ...
    return _softPatch(
      'workspace_members',
      `workspace_id=eq.${_enc(workspaceId)}&user_id=eq.${_enc(userId)}`,
      { role: 'viewer', visible_client_ids: [] }
    );
  }
  ...
  async function lookupApproval(token) {
    ...
    const rows = await _softGet(
      'client_approvals',
      `approval_token=eq.${_enc(token)}&limit=1`
    );
    const r = rows[0] || null;
    if (!r) return null;
