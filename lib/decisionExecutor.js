  ...
  try {
    const r = await fetch(`${_selfBase()}${path}`, {
      ...
      headers: {
        ...
        // Must be 'x-webhook-secret' — that is the header requireAuthOrWebhookSecret
        // reads. 'X-Internal-Secret' is ignored → loopback got 401 and approved
        // War Room decisions silently never executed.
...
async function execute(decision, { sbPatch, logger, internalSecret } = {}) {
  ...
  if (!route) {
    if (typeof sbPatch === 'function') {
      await sbPatch('decision_logs', `id=eq.${encodeURIComponent(decision.id)}`, {
        executed: false,
