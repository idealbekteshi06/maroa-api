...
// Embedding is delegated to lib/embeddingProviders/. The active provider
// (OpenAI text-embedding-3-small when configured, stub otherwise) is
// picked once at boot. Default dim is 384 — matches migration 061's
// vector(384) column. OpenAI is called with `dimensions: 384` to keep
// the storage shape compatible without resizing the column.
...
function createPerformanceMemory({ sbGet, callClaude, logger } = {}) {
  ...
  /**
   * Generate an embedding for a piece of text. Delegates to the active
   * provider in lib/embeddingProviders/:
   *
   *   - OpenAI text-embedding-3-small at dim=384 when OPENAI_API_KEY is set
   *   - Stub (deterministic token-hash, dim=384) otherwise
   *
   * Returns Float32Array or null on failure.
   */
  async function embed(text) {
    if (!text) return null;
    try {
      const providers = require('./embeddingProviders');
      return await providers.embed(text);
    } catch (e) {
      logger?.warn?.('performance-memory.embed', null, 'embedding failed', { error: e.message });
      return null;
    }
  }
