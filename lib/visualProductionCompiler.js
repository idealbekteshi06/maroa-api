...
const MODEL_ROUTING = Object.freeze({
  ...
  meta_ad_video: {
    quality: {
      primary: { name: 'Veo', path: '/google/veo/v3-1', cost: 0.40 },
  ...
  ugc_ad_video: {
    quality: {
      primary: { name: 'Cinema Studio 3.5', path: '/higgsfield-ai/cinema-studio/v3-5', cost: 0.35 },
      fallback: { name: 'Sora', path: '/higgsfield-ai/sora/standard', cost: 0.50 },
    },
    cost: {
      primary: { name: 'DOP Turbo', path: '/higgsfield-ai/dop/turbo', cost: 0.20 },
      fallback: { name: 'Seedance', path: '/bytedance/seedance/v1/pro/image-to-video', cost: 0.18 },
    },
  },
  motion_graphic: {
    quality: {
      primary: { name: 'Vibe Motion', path: '/higgsfield-ai/vibe-motion/standard', cost: 0.10 },
...
function makeVisualProductionCompiler(deps = {}) {
  ...
  function _buildShotList({ intent, format, hookDirecting }) {
    if (!format.duration_sec) {
      return [
        { description: 'Single hero frame — product centered, brand-consistent lighting', camera: 'medium-close', motion: 'static', duration_sec: null },
      ];
    }
    ...
    if (total <= 30) {
      return [
        { description: `Hook (${hookDirecting.opener})`, camera: 'close-up', motion: hookDirecting.motion, duration_sec: Math.min(3, total) },
        { description: 'Demonstration — product in use with on-screen benefit text', camera: 'medium', motion: 'subtle push-in', duration_sec: Math.max(7, total - 6) },
        { description: 'CTA — clear single action, product hero, brand logo subtle', camera: 'medium-close', motion: 'static or slow zoom', duration_sec: 3 },
      ];
    }
    // Longer: hook + 3 beats + CTA
    return [
      { description: `Hook (${hookDirecting.opener})`, camera: 'close-up', motion: hookDirecting.motion, duration_sec: 3 },
      { description: 'Beat 1 — problem framing', camera: 'medium', motion: 'static', duration_sec: Math.floor((total - 6) / 3) },
      { description: 'Beat 2 — solution reveal', camera: 'medium-close', motion: 'reveal/orbit', duration_sec: Math.floor((total - 6) / 3) },
      { description: 'Beat 3 — proof / benefit on screen', camera: 'medium', motion: 'subtle motion', duration_sec: Math.floor((total - 6) / 3) },
  ...
  function _buildPromptFromBrief({ industry, intent, hookType, offer, audience, brandDna, hookDirecting }) {
    ...
    if (offer) lines.push(`Offer to highlight: ${typeof offer === 'string' ? offer : (offer.name || offer.description || '')}.`);
  ...
  function _buildNegativePrompt() {
    return 'low quality, distorted faces, mangled hands, blurry, watermark, ' +
  ...
  function _buildQaChecklist({ channel, industry, hookType, format }) {
    ...
    if (format.duration_sec) checks.push({ check: 'Duration within window', expected: `${format.duration_sec}s`, passed: null });
    ...
    if (industry) {
      ...
      checks.push({ check: `Industry compliance — ${industry}`, regulator: 'see services/prompts/compliance', passed: null });
    }
