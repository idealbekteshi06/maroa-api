...
function createDetector({ logger, sentry = null } = {}) {
  ...
  function bucketFor(ip) {
    if (!state.has(ip)) {
      // OOM Protection: cap at MAX_IPS. Map iteration is in insertion order,
      // so keys().next().value evicts the oldest entry. With sweep() running
      // on a 5-min interval, eviction is rare under normal load.
