...
function createDetector({ logger, sentry = null } = {}) {
  ...
  function bucketFor(ip) {
    if (!state.has(ip)) {
      // OOM Protection: cap at MAX_IPS. Evict by least-recent ACTIVITY (not
      // insertion order) so an active attacker's bucket isn't dropped while
      // idle ones linger — which would reset their abuse counters.
