...
function securityHeaders({ env, defaultCspMode = 'api' } = {}) {
  ...
  return function securityHeadersMiddleware(req, res, next) {
    ...
    res.end = function patchedEnd(...args) {
      // Guard headersSent: streaming/SSE responses (e.g. /webhook/dashboard-events)
      // flush headers before res.end, so setHeader here would throw
      // ERR_HTTP_HEADERS_SENT. Only set CSP if headers haven't gone out yet.
      const mode = res.locals?.cspMode || defaultCspMode;
      const policy = CSP_PROFILES[mode];
      if (!res.headersSent && policy && !res.getHeader('Content-Security-Policy')) {
