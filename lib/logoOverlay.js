 ...
 *
 * Pure-JS (jimp@0.22, no native build). The compositing core works on Buffers
...
async function compositeLogo(baseBuffer, logoBuffer, opts = {}) {
  ...
  const baseW = base.getWidth();
  const baseH = base.getHeight();
  const targetLogoW = Math.max(1, Math.round(baseW * o.scale));
  logo.resize(targetLogoW, Jimp.AUTO);
  if (o.opacity < 1) logo.opacity(o.opacity);

  const margin = Math.round(baseW * o.marginRatio);
  const { x, y } = placement(o.position, baseW, baseH, logo.getWidth(), logo.getHeight(), margin);
