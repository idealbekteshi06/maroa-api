...
function makeMarketingGraph(deps = {}) {
  ...
  async function upsertEntity({ businessId, type, subtype, title, description, attrs, externalId, source, status }) {
    ...
    if (externalId) {
      ...
      if (existing.length) {
        ...
        return _softPatch('marketing_graph_entities', `id=eq.${existing[0].id}`, updates);
      }
    }
  ...
  async function getNeighbors({ businessId, entityId, edgeType, direction = 'out' }) {
    ...
    let filter =
      `business_id=eq.${_encode(businessId)}&${directionCol}=eq.${_encode(entityId)}` +
    ...
    const entities = await _softGet('marketing_graph_entities', `id=in.(${inList})&select=*`);
  ...
  async function recordClaim({ businessId, claimText, claimType, evidenceUrl, complianceFlags }) {
    ...
    if (existing.length) {
      return _softPatch('claims_library', `id=eq.${existing[0].id}`, {
  ...
  async function recordAudience({ businessId, name, segmentType, sourcePlatform, platformId, sizeEstimate, spec }) {
  ...
  async function recordExperiment({ businessId, name, hypothesis, variantCount, primaryMetric, budgetUsd }) {
  ...
  async function completeExperiment({ id, winnerCreativeId, confidenceScore, liftPct, conclusion, spendUsd }) {
    if (!id) return null;
    if (!(await isHealthy())) return null;
    const updates = {
      status: 'completed',
