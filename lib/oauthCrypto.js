...
function getKey() {
  ...
  if (buf.length !== 32) {
    throw new Error(
      `OAUTH_TOKEN_ENC_KEY must decode to 32 bytes (got ${buf.length}). Generate with: openssl rand -hex 32`
    );
  }
  cachedKey = buf;
  return cachedKey;
 ...
 *
 * SECURITY (post-2026-05-11 Antigravity adversarial review):
 * Previously this caught decrypt() errors and fell back to legacy
 * plaintext. That was a crypto downgrade vector — an attacker with DB
 * write access could overwrite `*_enc` with garbage (forcing decrypt
 * to throw) while injecting their own plaintext into the legacy column.
 * The app would then read attacker-controlled plaintext.
 *
 * Now: fall back to legacy ONLY when `*_enc` is null/undefined/empty
 * (meaning the row hasn't been backfilled yet). If `*_enc` IS set but
 * decrypt throws (tamper / wrong key / corruption), we propagate the
 * error so the caller can refuse to act on the data.
 *
 * Once migration 060 drops the legacy plaintext columns, the fallback
 * dies entirely.
 */
function readToken(row, legacyColumn) {
  if (!row) return null;
  const enc = row[`${legacyColumn}_enc`];
  // CASE 1: encrypted column populated — decrypt or THROW. Never
  // silently fall back to plaintext when an encrypted blob is present.
  if (enc) {
    // If an encrypted token is present, it MUST decrypt successfully.
    // Catching and falling back to legacy allows a crypto downgrade attack.
    return decrypt(enc);
  }
  // CASE 2: encrypted column not populated yet (pre-backfill). Use
  // legacy plaintext as the transition path.
