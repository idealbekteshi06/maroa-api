...
async function checkCostCap({ businessId, sbGet }) {
  ...
  try {
    ...
    const bizRows = await sbGet('businesses', `id=eq.${businessId}&select=plan`).catch(() => []);
    ...
    const rows = await sbGet('llm_cost_logs',
      `business_id=eq.${businessId}&created_at=gte.${sinceISO}&select=cost_usd&limit=10000`
    ).catch(() => null);
