...
async function probeSupabase(sbGet) {
  ...
  try {
    ...
    // (Asia/EU regions especially). 1000ms was too aggressive and produced
    // false-positive 503s when /readyz fired during regular traffic spikes.
    // See audit 2026-05-12.
 ...
 * Returns ok=false if any registry fails to load or returns 0 modules. Cheap
 * (in-process require + count), runs in <5ms. Only relevant when
 * AGENCY_PIPELINE_ENABLED is on; otherwise reported as "skipped".
 */
...
function registerHealthRoutes({ app, sbGet, logger }) {
  ...
  app.get('/readyz', async (req, res) => {
    ...
    const [supabase, anthropic, inngest, higgsfield, wave60] = await Promise.all([
      probeSupabase(sbGet),
      probeAnthropic(),
      probeInngest(),
      probeHiggsfield(),
    ...
    const checks = { supabase, anthropic, inngest, higgsfield, wave60 };
