...
function registerHealthRoutes({ app, sbGet, logger, inngestClient }) {
  ...
  app.get('/readyz', async (req, res) => {
    ...
    try {
      ...
      // Table existence (inbox_routing_settings, quality_gate_runs) — not _migrations ledger.
    ...
    const softWarnings = ['anthropic', 'wave60', 'higgsfield', 'inngest', 'dlq', 'migrations'].filter((k) => {
      const c = checks[k];
      if (!c) return false;
