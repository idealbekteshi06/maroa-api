...
function registerHealthRoutes({ app, sbGet, logger, inngestClient }) {
  ...
  app.get('/readyz', async (req, res) => {
    ...
    try {
      const { probeCriticalMigrations } = require('./platformOps');
      // Table existence via PostgREST (per-table select/limit=1) — not _migrations ledger.
      migrations = await probeCriticalMigrations(sbGet);
    ...
    // Real-generation heartbeat. The Anthropic probe above pings /v1/models —
    // a FREE endpoint that stays green even with exhausted credits — so it
    // CANNOT detect a generation outage. The 30-min canary makes a real Haiku
    // call; we read its last verdict here. Soft-only: a failed/stale canary
    // warns but never 503s (a canary bug must not take the app down).
    let generation_canary = { ok: true, skipped: true };
    try {
      generation_canary = require('./generationCanary').readCanary();
    } catch (e) {
      generation_canary = { ok: true, skipped: true, reason: e.message };
    }

    const checks = { supabase, anthropic, inngest, higgsfield, wave60, dlq, migrations, breakers, generation_canary };
    // Hard-fail: Supabase only (DB required to serve). Anthropic, Wave 60,
    // Higgsfield, etc. are soft — degraded AI must not block Railway routing.
    const hardFailures = ['supabase'].filter((k) => checks[k] && !checks[k].ok);
    const softWarnings = [
      'anthropic',
      'wave60',
      'higgsfield',
