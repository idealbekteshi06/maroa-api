...
async function probeInngest({ inngestClient } = {}) {
  ...
  if (inngestClient && typeof inngestClient === 'object') {
    ...
    if (fnCount === 0) {
      return { ok: false, reason: 'inngest_no_functions_registered' };
    }
...
async function probeHiggsfield() {
  ...
  return _cached('higgsfield', async () => {
    try {
      // Higgsfield exposes a GET /v1/models or similar low-cost endpoint.
      // If it returns 401, our keys are bad. If 5xx, provider is degraded.
      const r = await withTimeout(
        fetch('https://api.higgsfield.io/v1/models', {
          method: 'GET',
          headers: {
            'X-Api-Key-Id': id,
      ...
      // 2xx and 401 both count as "service up" — 401 means our creds are
      // bad but the service responded. Network failures + 5xx are real.
      if (r.ok) return { ok: true, status: r.status };
    ...
    async () => {
      try {
        ...
        const rows = await withTimeout(
          sbGet('inngest_dlq', `created_at=gte.${sinceISO}&select=id&limit=1000`),
          1500,
          'inngest_dlq'
        );
...
function registerHealthRoutes({ app, sbGet, logger, inngestClient }) {
  ...
  app.get('/readyz', async (req, res) => {
    ...
    const checks = { supabase, anthropic, inngest, higgsfield, wave60, dlq, breakers };
    ...
    const softWarnings = ['higgsfield', 'inngest', 'dlq']
      .filter((k) => checks[k] && !checks[k].ok && !checks[k].skipped);
