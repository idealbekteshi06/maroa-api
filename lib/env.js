...
const schema = z
  .object({
    ...
    SUPABASE_KEY: cleanedString().refine((v) => typeof v === 'string' && v.length >= 20, 'SUPABASE_KEY required (service role key, ≥20 chars)'),
    ...
    N8N_WEBHOOK_SECRET: cleanedString().refine((v) => typeof v === 'string' && v.length >= 16, 'N8N_WEBHOOK_SECRET must be ≥16 chars'),
