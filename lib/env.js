...
const schema = z
  .object({
    ...
    // SSE stream-ticket signing (lib/streamTicket.js). Optional — falls back
    // to N8N_WEBHOOK_SECRET so deploys need no new config; set it to rotate
    // ticket signing independently of the webhook secret.
    STREAM_TICKET_SECRET: optionalString(),
...
function parse() {
  ...
  warnPaddleSandboxInProduction(env);
...
const LIVE_FLAGS = ['META_AD_LAUNCH_LIVE', 'META_PUBLISH_LIVE', 'GOOGLE_ADS_LIVE', 'TIKTOK_ADS_LIVE'];
...
/**
 * PADDLE_ENV defaults to sandbox. In a production deploy that means customers
 * "subscribe" against the Paddle sandbox and get upgraded to paid plans while
 * $0 is actually collected — a silent revenue-off switch (2026-07 pre-launch
 * audit P0). Warn LOUDLY at boot rather than exit: staging/prod-like envs may
 * legitimately run sandbox for end-to-end checkout tests.
 */
function warnPaddleSandboxInProduction(env) {
  const isProd = env.NODE_ENV === 'production' || String(env.RAILWAY_ENVIRONMENT || '').toLowerCase() === 'production';
  if (!isProd) return;
  if (String(env.PADDLE_ENV || 'sandbox').toLowerCase() !== 'production') {
    console.error(
      '\n[env] ⚠️  PADDLE_ENV is not "production" in a production deploy.\n' +
        '       Checkout will run against the Paddle SANDBOX — customers get\n' +
        '       upgraded but NO money is collected. Set PADDLE_ENV=production\n' +
        '       (plus the live PADDLE_API_KEY / PADDLE_WEBHOOK_SECRET / price\n' +
        '       IDs) before taking paid traffic.\n'
    );
  }
}
