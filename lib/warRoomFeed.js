...
function makeWarRoomFeed(deps = {}) {
  const { sbGet, workspaces, marketingGraph, decisionLog, logger, metrics } = deps;
  ...
  async function getWorkspaceFeed(workspaceId, { perClientLimit = 10 } = {}) {
    ...
    const totalRunningExperiments = clientFeeds.reduce((s, c) => s + (c.experiments_running?.length || 0), 0);
  ...
  async function summarizeForDashboard(workspaceId) {
    ...
    const outcomeScores = decisions.map((d) => d.outcome_score).filter((v) => typeof v === 'number');
